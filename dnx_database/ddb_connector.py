...
from dnx_configure.dnx_constants import FIVE_MIN, ONE_DAY
...
class DBConnector:
    ...
    # NOTE: replaced with context manager. keeping until all code removes this call
    def Connect(self):
        self.conn = pg8000.connect(database='dnxfirewall', user=USER, password=PASSWORD, host='127.0.0.1', port=5432)
        self.c = self.conn.cursor()
    ...
    def dashboard_query_top(self, count, *, table, action):
        if (action in ['allowed', 'blocked']):
            self.c.execute(f'select * from {table} where action=%s order by count desc limit 20', (action,))
        elif (action in ['all']):
