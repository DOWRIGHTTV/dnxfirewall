...
__all__ = (
    'ProxyConfiguration', 'ServerConfiguration', 'CFG_PROFILE'
)
...
class CFG_PROFILE(NamedTuple):
    ...
    whitelist: ClassVar[DNS_WHITELIST]
    blacklist: ClassVar[DNS_BLACKLIST]
...
class ProxyConfiguration(ConfigurationMixinBase):
    ...
    @cfg_read_poller('profiles/profile_x', profiles=(1, 15), cfg_type='security/dns')
    def _get_proxy_settings(self, profile_idx: int, proxy_config: ConfigChain) -> None:
    ...
    # deprecated:: dns white/blacklist are disabled for rework.
    # idea:: this is being reworked, but as an idea, we could use the standard read poller
    #  then have a separate thread that will check for timeouts in a set interval.
    #    - this will make a timeout potentially go past the configured amount by the interval amount.
    #  then we can have a lazy check on the webui, that when loading the page, will check for timeouts, and filter out.
    #    - if the user adds a new rule, it will also apply to disk any timeouts that have occurred.
    #  if we do this the proxy will not have to write timeouts back to disk, and will just maintain that in memory.
    # @cfg_write_poller
...
class ServerConfiguration(ConfigurationMixinBase):
    ...
    '''
    protocol: ClassVar[NETWORK_PROTOCOL] = PROTO_NOT_SET
    ...
    public_resolvers: ClassVar[DNS_SERVERS] = DNS_SERVERS(
        {'ip_address': None, PROTO_UDP: None, PROTO_DNS_TLS: None},
        {'ip_address': None, PROTO_UDP: None, PROTO_DNS_TLS: None}
    )
    ...
    def _get_server_settings(self, server_config: ConfigChain) -> None:
        ...
        self.__class__.protocol = PROTO_DNS_TLS if server_config['tls->enabled'] else PROTO_UDP
        ...
        for i, resolver in enumerate(loaded_resolvers):
            ...
            if (resolver['ip_address'] != configured_resolvers[i]['ip_address']):
                ...
                configured_resolvers[i].update({
                    ...
                    PROTO_UDP: False, PROTO_DNS_TLS: False
                })
    ...
    def _udp_reachability(self, udp_query: bytes, udp_sock: Socket_T):
        ...
        if (self.protocol is not PROTO_UDP and not self.__class__.udp_fallback):
            return

        downed_servers: list[tuple[int, str]] = [
            (idx, server['ip_address']) for idx, server in enumerate(public_resolvers) if not server[PROTO_UDP]
        ]
        ...
        for idx, server in downed_servers:
            ...
            udp_sock.sendto(udp_query, (server, PROTO_DNS))
            try:
                udp_sock.recv(1024)
            except OSError:
                continue
            ...
            status_change = True
            public_resolvers[idx][PROTO_UDP] = True
    ...
    def _tls_reachability(self, tls_context: SSLContext):
        ...
        if (self.protocol is not PROTO_DNS_TLS):
            return

        downed_servers: list[tuple[int, str]] = [
            (idx, server['ip_address']) for idx, server in enumerate(public_resolvers) if not server[PROTO_DNS_TLS]
        ]
        ...
        for idx, secure_server in downed_servers:
            ...
            try:
                secure_socket.connect((secure_server, PROTO_DNS_TLS))
            except OSError:
                return False
                ...
                public_resolvers[idx][PROTO_DNS_TLS] = True
