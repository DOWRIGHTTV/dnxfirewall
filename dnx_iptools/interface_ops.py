...
from dnx_gentools.file_operations import load_data, load_configuration, ConfigurationManager, json_to_yaml
...
def set_wan_interface(intf_type: INTF = INTF.DHCP):
    ...
    with ConfigurationManager('system') as dnx:
        ...
        wan_ident = dnx_settings['interfaces->builtins->wan->ident']

        # template used to generate yaml file with user configured fields
        intf_template = load_data('intf_config_template.cfg', filepath='dnx_system/interfaces')
        ...
        if (intf_type is INTF.STATIC):
            wan_intf = intf_template['network']['ethernets'][wan_ident]
