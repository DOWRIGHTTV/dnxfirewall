...
cdef public struct TrieMap_R:
    size_t      len
    TrieRange  *ranges

cdef public struct TrieRange:
    uint32_t    key
    uint32_t    netid
    uint32_t    bcast
    uint8_t     country
...
cdef public struct TrieMap_V:
    size_t      len
    TrieValue  *values

cdef public struct TrieValue:
    uint32_t    key
    uint32_t    value


cdef public class HashTrie_Value[object HashTrie_Value, type HashTrie_Value_T]:
    cdef:
        TrieMap_V  *TRIE_MAP
        uint32_t    max_width
