...
cdef class HashTrie_Value:
    ...
    def py_search(self, uint32_t trie_key):
        ...
        cdef:
            ...
            # uint32_t trie_key = <uint32_t>host[0]
            # uint32_t value_id  = <uint32_t>host[1]
    ...
    cdef uint8_t search(self, uint32_t trie_key):
    ...
    cpdef void generate_structure(self, list py_trie, Py_ssize_t py_trie_len):
        ...
        for i in range(py_trie_len):
            ...
            # at least 1 multi-val is present, so reallocating memory to include new multi-vals
                ...
                trie_map_container.ranges = <TrieRange*>realloc(
                    trie_map_container.ranges, sizeof(TrieRange) * (trie_map_container.len + 1)
                )

            trie_multival = &trie_map_container.ranges[trie_map_container.len]
