            ...
            try:
                ...
                if (packet.qtype == 1):
                    if (self.protocol == UDP):
                        threading.Thread(target=self.UDPRelay, args=(data_from_client, client_address)).start()
                    elif (self.protocol == TCP):
                        threading.Thread(target=self.TLSQueue, args=(data_from_client, client_address)).start()
